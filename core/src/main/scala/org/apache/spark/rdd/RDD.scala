...
abstract class RDD[T: ClassTag](
  ...
  def saveAsTextFile(path: String) {
    ...
    //RDD调用saveAsTextFile方法时还会产生一个RDD
    val r = this.mapPartitions { iter =>
      val text = new Text()
      iter.map { x =>
        text.set(x.toString)
        (NullWritable.get(), text)   //因为在textFile 中将key去掉了，所以这里需要补上key，才可以调用saveAsHadoopFile
      }
    }
    //saveAsHadoopFile
    RDD.rddToPairRDDFunctions(r)(nullWritableClassTag, textClassTag, null)  //变量r是最后一个RDD，所以记为final RDD
