  ...
  with Serializable
{
  ...
  def saveAsHadoopFile(
    ...
    for (c <- codec) {
      ...
      hadoopConf.set("mapred.output.compress", "true") //压缩
      hadoopConf.setMapOutputCompressorClass(c)
      hadoopConf.set("mapred.output.compression.codec", c.getCanonicalName)  //编码格式
   ...
   * MapReduce job.
   */
  //将rdd存储到任何hadoop支持的存储系统，需要使用hadoop的jobConf
  def saveAsHadoopDataset(conf: JobConf) {
    ...
    writer.preSetup()
    //这是一个函数，Iterator把一个分区的数据传过去    iter 数据迭代器
    val writeToFile = (context: TaskContext, iter: Iterator[(K, V)]) => {
      ...
      var recordsWritten = 0L
      try {
        while (iter.hasNext) { //一个分区的数据迭代
          val record = iter.next()     //得到一条数据
    ...
    //开始提交任务
    self.context.runJob(self, writeToFile) //self类型是RDD,是final RDD，这个RDD可以通过依赖关系进行切分Stage
    writer.commitJob()
  }
