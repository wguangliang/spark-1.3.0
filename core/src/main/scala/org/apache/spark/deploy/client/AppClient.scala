...
/**
 * //这是一个接口
 * //负责为Application与spark master进行通信
 * @param actorSystem
 * @param masterUrls
...
private[spark] class AppClient(
  ...
  class ClientActor extends Actor with ActorLogReceive with Logging {
    ...
    override def receiveWithLogging = {
      //接收到master发送的注册application成功的消息
      case RegisteredApplication(appId_, masterUrl) =>
        appId = appId_  //master分配的注册applicationid
        registered = true
      ...
      case ApplicationRemoved(message) =>
        ...
        context.stop(self)
      //client接收到master发送过来的Executor启动的消息（client不负责启动Executor，只是告知Executor已经启动了）
      case ExecutorAdded(id: Int, workerId: String, hostPort: String, cores: Int, memory: Int) =>
        val fullId = appId + "/" + id
        logInfo("Executor added: %s on %s (%s) with %d cores".format(fullId, workerId, hostPort,
          cores))
       //client在向master发送已知Executor状态改变的消息
