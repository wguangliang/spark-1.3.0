...
object SparkSubmit {
  ...
  def main(args: Array[String]): Unit = {
    ...
    appArgs.action match {
      case SparkSubmitAction.SUBMIT => submit(appArgs)   //提交任务
      case SparkSubmitAction.KILL => kill(appArgs)       //kill任务
      case SparkSubmitAction.REQUEST_STATUS => requestStatus(appArgs) //查看任务状态
    }
  }
  ...
  private def runMain(
    ...
    try {
      mainClass = Class.forName(childMainClass, true, loader)  //反射，调用用户写的--class类，拿到该类对象
    } catch {
    ...
    val mainMethod = mainClass.getMethod("main", new Array[String](0).getClass) //调用该类的main方法
    ...
    try {
      mainMethod.invoke(null, childArgs.toArray)   //invoke执行
    } catch {
      case t: Throwable =>
        throw findCause(t)
    }
